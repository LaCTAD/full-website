...
class GFZapier {
	...
	private static function zapier_edit_page($form_id, $zap_id){
        ...
        if(rgpost("save")){
            ...
            if ($zap["url"] != rgpost("gform_zapier_url")){
            	$is_update = true;
			}
        ...
        <form method="post" id="gform_zapier_form">
        	...
        	<table class="form-table">
		        ...
		        <tr valign="top">
		            ...
		            <td>
		                <input type="checkbox" id="gf_zapier_conditional_enabled" name="gf_zapier_conditional_enabled" value="1" onclick="if(this.checked){jQuery('#gf_zapier_conditional_container').fadeIn('fast');} else{ jQuery('#gf_zapier_conditional_container').fadeOut('fast'); }" <?php echo rgar($zap['meta'], 'zapier_conditional_enabled') ? "checked='checked'" : ""?>/>
                        <label for="gf_zapier_conditional_enable"><?php _e("Enable", "gravityformszapier"); ?></label>
		                <br/>
                        <div style="height:20px;">
		                    <div id="gf_zapier_conditional_container" <?php echo !rgar($zap['meta'], 'zapier_conditional_enabled') ? "style='display:none'" : ""?>>
	...
	public static function send_form_data_to_zapier($entry = null, $form){
		...
		foreach ($zaps as $zap) {
			...
			if (!$is_entry || ($is_entry && self::conditions_met($form, $zap))){
				...
				$form_data = array("sslverify" => false, "ssl" => true, "body" => $json_body);
